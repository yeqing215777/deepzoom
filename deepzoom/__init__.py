...
class ImageCreator(object):
    ...
    def create(self, source, destination='index.dzi'):
        """Creates Deep Zoom image from source file and saves it to destination."""

        if self.file_extension(source) in self.photoshopfile():
            self.image = PSDImage.open(source).composite()
        else:
        ...
        image_files = os.path.join(_get_or_create_path(self.sp),
                                   _get_files_path(destination)
                                   )
        ...
        for level in range(self.descriptor.num_levels):
            ...
            for (column, row) in self.tiles(level):
                ...
                logger.info('save {} success.'.format(tile_path))
                if self.descriptor.tile_format == "jpg":
                    ...
                    tile.save(tile_file)
        # Create descriptor
        destination = os.path.join(self.sp, destination)
